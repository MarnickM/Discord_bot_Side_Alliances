...
captain_role_id = 1221545013209595945
garyID = 781062676557594625
kuxID = 1078626493023920178
ahmedID = 679724404757889066
kekID = 1184165487861563433
pawID = 347291231396691978
mattID = 651734654562336768
toshiID = 446364761089638403
st4si3kID = 463872361343614996
...
async def createwar(ctx, enemyAlliance=None):
  # if ctx.author.id != garyID and ctx.author.id != kekID and ctx.author.id != kuxID and ctx.author.id != ahmedID and ctx.author.id != pawID and ctx.author.id != mattID and ctx.author.id != st4si3kID and ctx.author.id != evelinID:
  #   await ctx.send("you can't use this command")
  #   return
...
async def refreshMainWp(ctx):
    ...
    try:
      response = requests.get(f"https://api.galaxylifegame.net/Alliances/get?name=zetsumetsu")
      data = response.json()
      if 'Members' in data:
